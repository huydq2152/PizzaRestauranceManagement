...
public class ServiceRegistryHttpMessageHandler(
    IServiceRegistry serviceRegistry,
    IOptions<ServiceDiscoverySettings> serviceDiscoverySettings)
    : DelegatingHandler
{
    private readonly ServiceDiscoverySettings _serviceDiscoverySettings = serviceDiscoverySettings.Value;
    private readonly Dictionary<string, string> _retrievedServices = new();
    ...
    {
        var discoveredHost = await serviceRegistry.GetServiceAddress(current.Host);
