...
public sealed class KitchenStepDefinitions
{
    ...
    public async Task ThenAnOrderWithIdentifierOrdShouldBeAddedToTheNewKitchenRequests(string p0)
    {
        ...
        var newKitchenRequests = await _kitchenDriver.GetNew();
    ...
    {
        await _kitchenDriver.Preparing(p0);
        await _kitchenDriver.PrepComplete(p0);
        await _kitchenDriver.BakeComplete(p0);
        await _kitchenDriver.QualityChecked(p0);
    }
