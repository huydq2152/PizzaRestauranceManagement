...
public class DeliveryDriver
{
    ...
    {
        var result = await _httpClient.GetAsync(new Uri($"{BaseUrl}/delivery/awaiting-collection"))
    ...
    public async Task AssignDriver(string orderIdentifier, string driverName)
    {
        ...
        var result = await _httpClient.PostAsync(new Uri(url), new StringContent(content, Encoding.UTF8, "application/json")).ConfigureAwait(false);
    ...
    {
        var result = await _httpClient.GetAsync(new Uri($"{BaseUrl}/delivery/driver/{driverName}/orders"))
            .ConfigureAwait(false);
