...
public class DriverCollectedOrderEventWorker(
    RabbitMqEventSubscriber eventSubscriber,
    ActivitySource source,
    DriverCollectedOrderEventHandler eventHandler)
    : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var subscription = await eventSubscriber.CreateEventConsumer(queueName, "delivery.driverCollectedOrder.v1");
        ...
        subscription.Consumer.ReceivedAsync += async (model, ea) =>
        {
            ...
            {
                var evtDataResponse =
                    await eventSubscriber.ParseEventFrom<DriverCollectedOrderEventV1>(ea.Body.ToArray());
