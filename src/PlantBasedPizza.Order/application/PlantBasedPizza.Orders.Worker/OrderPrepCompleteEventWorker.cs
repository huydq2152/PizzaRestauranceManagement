...
public class OrderPrepCompleteEventWorker(
    ...
    : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        var subscription = await eventSubscriber.CreateEventConsumer(queueName, "kitchen.orderPrepComplete.v1");
        ...
        subscription.Consumer.ReceivedAsync += async (model, ea) =>
        {
            ...
            {
                var evtDataResponse =
                    await eventSubscriber.ParseEventFrom<OrderPrepCompleteEventV1>(ea.Body.ToArray());

                using var processingActivity = source.StartActivity("processing-order-prep-complete-event",
