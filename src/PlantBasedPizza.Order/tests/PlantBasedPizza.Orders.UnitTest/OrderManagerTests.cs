...
public class OrderManagerTests
{
    ...
    public void CanCreateNewOrder_ShouldSetDefaultFields()
    {
        ...
        DomainEvents.Register<OrderCreatedEvent>((evt) => { createdOrder = evt.OrderIdentifier; });

        var order = Order.Core.Entities.Order.Create(DefaultOrderIdentifier, OrderType.Pickup,
            DefaultCustomerIdentifier);
    ...
    {
        var order = Order.Core.Entities.Order.Create(DefaultOrderIdentifier, OrderType.Delivery,
            DefaultCustomerIdentifier, new DeliveryDetails()
            {
                AddressLine1 = "TEST",
                Postcode = "XN6 7UY"
            });
    ...
    public void CanCreateAndCompletetOrder_ShouldBeCompleted()
    {
        ...
        DomainEvents.Register<OrderCompletedEvent>((evt) => { completedOrder = evt.OrderIdentifier; });
