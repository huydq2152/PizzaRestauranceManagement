...
[AsyncApi]
public class KitchenEventPublisher(IEventPublisher eventPublisher) : IKitchenEventPublisher
{
    ...
    {
        await eventPublisher.Publish(new KitchenConfirmedOrderEventV1
        {
    ...
    {
        await eventPublisher.Publish(new OrderBakedEventV1
        {
            OrderIdentifier = request.OrderIdentifier,
            KitchenIdentifier = request.KitchenRequestId
        });
    ...
    {
        await eventPublisher.Publish(new OrderPreparingEventV1
        {
            OrderIdentifier = request.OrderIdentifier,
            KitchenIdentifier = request.KitchenRequestId
        });
    ...
    {
        await eventPublisher.Publish(new OrderPrepCompleteEventV1
        {
            OrderIdentifier = request.OrderIdentifier,
            KitchenIdentifier = request.KitchenRequestId
        });
    ...
    {
        await eventPublisher.Publish(new OrderQualityCheckedEventV1
        {
            OrderIdentifier = request.OrderIdentifier,
            KitchenIdentifier = request.KitchenRequestId
        });
